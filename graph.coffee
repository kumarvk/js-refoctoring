Graph = do ->
  ...
  Graph = (data) ->
    ...
    @getXAxis = (xRange) ->
      d3.svg.axis().scale(xRange).tickSize(5).tickSubdivide true

    @getYAxis = (yRange) ->
      d3.svg.axis().scale(yRange).tickSize(5).orient('left').tickSubdivide true

    @setGraphForXAxis = (xAxis) ->
      @graph.append('svg:g').attr('class', 'x axis').attr('transform', 'translate(0,' + @height - (@margins.bottom) + ')').call xAxis
      return

    @setGraphForYAxis = (yAxis) ->
  ...
  Graph::render = ->
    xRange = @getXRange()
    xAxis = @getXAxis(xRange)
    @setGraphForXAxis xAxis
    yRange = @getYRange()
    yAxis = @getYAxis(yRange)
    @setGraphForYAxis yAxis
